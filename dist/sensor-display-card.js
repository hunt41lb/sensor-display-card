...
`,ft=6;class $t{constructor(t){}get _$AU(){return this._$AM._$AU}_$AT(t,e,i){this._$Ct=t,this._$AM=e,this._$Ci=i}_$AS(t,e){return this.update(t,e)}update(t,e){return this.render(...e)}}const yt="ontouchstart"in window||navigator.maxTouchPoints>0;const vt=(t=>(...e)=>({_$litDirective$:t,values:e}))(class extends $t{constructor(t){if(super(t),this.held=!1,t.type!==ft)throw new Error("actionHandler directive can only be used on elements")}render(t){return B}update(t,[e]){return this.element||(this.element=t.element,this.setupListeners()),this.options=e||{},B}setupListeners(){yt?(this.element.addEventListener("touchstart",this.handleTouchStart.bind(this),{passive:!0}),this.element.addEventListener("touchend",this.handleTouchEnd.bind(this)),this.element.addEventListener("touchcancel",this.handleTouchCancel.bind(this))):(this.element.addEventListener("mousedown",this.handleMouseDown.bind(this)),this.element.addEventListener("mouseup",this.handleMouseUp.bind(this)),this.element.addEventListener("click",this.handleClick.bind(this)))}handleTouchStart(t){this.held=!1,this.options.hasHold&&(this.holdTimeout=setTimeout(()=>{this.held=!0,this.fireEvent("hold")},500))}handleTouchEnd(t){this.holdTimeout&&(clearTimeout(this.holdTimeout),this.holdTimeout=void 0),this.held||this.handleClick(t)}handleTouchCancel(){this.held=!1,this.holdTimeout&&(clearTimeout(this.holdTimeout),this.holdTimeout=void 0)}handleMouseDown(t){this.held=!1,this.options.hasHold&&(this.holdTimeout=setTimeout(()=>{this.held=!0,this.fireEvent("hold")},500))}handleMouseUp(){this.holdTimeout&&(clearTimeout(this.holdTimeout),this.holdTimeout=void 0)}handleClick(t){t.stopPropagation(),this.held?this.held=!1:this.options.hasDoubleClick?this.dblClickTimeout?(clearTimeout(this.dblClickTimeout),this.dblClickTimeout=void 0,this.fireEvent("double_tap")):this.dblClickTimeout=setTimeout(()=>{this.dblClickTimeout=void 0,this.fireEvent("tap")},250):this.fireEvent("tap")}fireEvent(t){this.options.disabled||this.element.dispatchEvent(new CustomEvent("action",{bubbles:!0,composed:!0,detail:{action:t}}))}});let bt=class extends rt{setConfig(t){this._config=t}_valueChanged(t){if(!this._config||!this.hass)return;const e=t.target,i=e.configValue,s=t.detail?.value??e.value;if(this._config[i]===s)return;const o={...this._config,[i]:""===s?void 0:s};Object.keys(o).forEach(t=>{void 0===o[t]&&delete o[t]}),this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:o},bubbles:!0,composed:!0}))}render(){return this.hass&&this._config?I`
  ...
  `,t([dt({attribute:!1})],bt.prototype,"hass",void 0),t([ut()],bt.prototype,"_config",void 0),bt=t([ht("sensor-display-card-editor")],bt);console.info("%c SENSOR-DISPLAY-CARD %c v1.0.0 ","color: white; background: #3498db; font-weight: bold;","color: #3498db; background: white; font-weight: bold;");let At=class extends rt{constructor(){super(...arguments),this._motionActive=!1}static getConfigElement(){return document.createElement("sensor-display-card-editor")}static getStubConfig(){return{type:"custom:sensor-display-card",light_entity:"light.living_room",name:"Living Room",icon:"mdi:lightbulb"}}setConfig(t){if(!t)throw new Error("Invalid configuration");if(!t.light_entity)throw new Error("You must define a light_entity");this._config={tap_action:{action:"toggle"},hold_action:{action:"more-info"},double_tap_action:{action:"more-info"},icon:"mdi:lightbulb",...t},this._hass&&this._updateStates()}set hass(t){this._hass=t,this._updateStates()}_updateStates(){if(!this._hass||!this._config)return;const t=this._hass.states[this._config.light_entity];if(this._lightEntity!==t&&(this._lightEntity=t),this._config.temp_sensor){const t=this._hass.states[this._config.temp_sensor]?.state;this._tempValue!==t&&(this._tempValue=t)}if(this._config.humidity_sensor){const t=this._hass.states[this._config.humidity_sensor]?.state;this._humidityValue!==t&&(this._humidityValue=t)}if(this._config.power_sensor){const t=this._hass.states[this._config.power_sensor]?.state;this._powerValue!==t&&(this._powerValue=t)}if(this._config.motion_sensor){const t="on"===this._hass.states[this._config.motion_sensor]?.state;this._motionActive!==t&&(this._motionActive=t)}}_handleAction(t){if(!this._hass||!this._config)return;const e=t.detail.action;if(!this._config[`${e}_action`])return;const i=new Event("hass-action",{bubbles:!0,composed:!0});i.detail={config:{entity:this._config.light_entity,tap_action:this._config.tap_action,hold_action:this._config.hold_action,double_tap_action:this._config.double_tap_action},action:e},this.dispatchEvent(i)}render(){if(!this._config||!this._hass)return I`<ha-card><div class="unavailable">Loading...</div></ha-card>`;if(!this._lightEntity)return I`
        <ha-card>
          <div class="unavailable">Entity not found: ${this._config.light_entity}</div>
        </ha-card>
      `;const t="on"===this._lightEntity.state,e=this._lightEntity.attributes.rgb_color,i=e&&t?`background-color: rgba(${e.join(",")}, 0.2);`:"",s=e&&t?`color: rgb(${e.join(",")});`:"",o=this._config.name||this._lightEntity.attributes.friendly_name||this._config.light_entity,n=this._config.icon||"mdi:lightbulb";return I`
      <ha-card
      ...
      >
        ...
        <!-- Sensor Values -->
        <div class="sensors">
          ${void 0!==this._tempValue?I`<span class="sensor-temp">${parseFloat(this._tempValue).toFixed(0)}°</span>`:q}
          ${void 0!==this._humidityValue?I`<span class="sensor-humidity">${parseFloat(this._humidityValue).toFixed(0)}%</span>`:q}
          ${void 0!==this._powerValue?I`<span class="sensor-power">${parseFloat(this._powerValue).toFixed(0)}W</span>`:q}
        </div>

        <!-- Motion Sensor -->
        ${this._config.motion_sensor?I`
              <div class="motion">
                ${this._motionActive?I`<ha-icon class="motion-active" icon="mdi:motion-sensor"></ha-icon>`:q}
              </div>
            `:q}
      </ha-card>
    `}getCardSize(){return 2}getGridOptions(){return{rows:2,columns:6,min_rows:2,min_columns:3}}getLayoutOptions(){return{grid_rows:2,grid_columns:2,grid_min_rows:2,grid_min_columns:2}}};At.styles=gt,t([ut()],At.prototype,"_lightEntity",void 0),t([ut()],At.prototype,"_tempValue",void 0),t([ut()],At.prototype,"_humidityValue",void 0),t([ut()],At.prototype,"_powerValue",void 0),t([ut()],At.prototype,"_motionActive",void 0),At=t([ht("sensor-display-card")],At),window.customCards=window.customCards||[],window.customCards.push({type:"sensor-display-card",name:"Sensor Display Card",description:"A card displaying RGB lights with temperature, humidity, power, and motion sensors",preview:!0,documentationURL:"https://github.com/YOURUSERNAME/sensor-display-card"});export{At as SensorDisplayCard};
